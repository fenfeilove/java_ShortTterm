...
function dhtmlXGridCellObject(a) {
	...
	this.getFont = function () {
		...
		if (this.cell.style.fontWeight ==
			"bold" || this.cell.parentNode.style.fontWeight == "bold")
			arOut[1] = "bold";
	...
	this.setChecked = function (a) {
		if (this.isCheckbox())
			a != "true" && a != 1 && (a = !1), this.cell.firstChild.checked = a
	};
	this.setDisabled = function (a) {
		a != "true" && a != 1 && (a = !1);
		if (this.isCheckbox())
			this.cell.firstChild.disabled = a, this.disabledF && this.disabledF(a);
...
function eXcell() {
	...
	this.getPosition = function (a) {
		for (var b = a, c = 0, d = 0; b.tagName != "BODY"; )
...
function eXcell_ed(a) {
	...
	this.edit = function () {
		...
		_isIE && (this.obj.focus(), this.obj.blur());
		this.obj.focus()
	};
	this.getValue = function () {
...
function eXcell_ch(a) {
	...
	this.changeState = function (a) {
		...
		if (this.grid.isEditable && !this.cell.parentNode._locked && !this.isDisabled())
			this.grid.callEvent("onEditCell", [0, this.cell.parentNode.idd, this.cell._cellIndex]) ? (this.val = this.getValue(), this.val == "1" ? this.setValue("0") : this.setValue("1"), this.cell.wasChanged = !0, this.grid.callEvent("onEditCell", [1, this.cell.parentNode.idd, this.cell._cellIndex]), this.grid.callEvent("onCheckbox", [this.cell.parentNode.idd, this.cell._cellIndex, this.val !=
...
eXcell_ch.prototype.setValue = function (a) {
	...
	if (a && (a = a.toString()._dhx_trim(), a == "false" || a == "0"))
		a = "";
...
function eXcell_ra(a) {
	...
	this.changeState = function (a) {
		if (!(a === !1 && this.getValue() == 1) && this.grid.isEditable && !this.cell.parentNode._locked)
			this.grid.callEvent("onEditCell", [0, this.cell.parentNode.idd,
...
function eXcell_txt(a) {
	...
	this.edit = function () {
		...
		if (_isOpera)
			this.obj.onkeypress = function (a) {
				if (a.keyCode == 9)
					return !1
			};
		this.obj.onkeydown = function (a) {
			var b = a || event;
			if (b.keyCode == 9)
				return globalActiveDHTMLGridObject.entBox.focus(), globalActiveDHTMLGridObject.doKey({
					keyCode : b.keyCode,
					shiftKey : b.shiftKey,
					srcElement : "0"
				}), !1
		...
		if (_isFF)
			this.obj.firstChild.style.width = parseInt(this.obj.style.width) + "px", this.obj.firstChild.style.height = this.obj.offsetHeight - 3 + "px";
		if (_isIE)
			this.obj.select(), this.obj.value = this.obj.value;
	...
	this.getValue = function () {
		return this.obj ? _isFF ? this.obj.firstChild.value : this.obj.value : this.cell._clearCell ? "" : typeof this.cell._brval != "undefined" ? this.cell._brval : this.grid.multiLine ? this.cell._brval || this.cell.innerHTML.replace(/<br[^>]*>/gi, "\n")._dhx_trim() : this.cell._brval || this.cell.innerHTML
	}
}
...
eXcell_txt.prototype.setValue = function (a) {
	this.cell._brval = a;
	!a || a.toString()._dhx_trim() == "" ? (a = "&nbsp;", this.cell._clearCell = !0) : this.cell._clearCell = !1;
	!this.grid.multiLine || this.cell._clearCell ? this.setCValue(a, this.cell._brval) : this.setCValue(a.replace(/\n/g, "<br/>"), a)
};
function eXcell_co(a) {
	if (a)
		this.cell = a, this.grid = this.cell.parentNode.grid, this.combo = this.cell._combo || this.grid.getCombo(this.cell._cellIndex), this.editable = !0;
	...
	this.edit = function () {
		...
		this.obj.onkeyup = function (a) {
			...
			if (!(b == 38 || b == 40 || b == 9))
				for (var d = this.readonly ? String.fromCharCode(b) : this.value, f = c.list.options, e = 0; e < f.length; e++)
					if (f[e].text.indexOf(d) == 0)
						return f[e].selected = !0
		};
		...
		if (!this.editable)
			this.obj.style.visibility = "hidden", this.list.focus(), this.list.onkeydown = function (a) {
				a = a || window.event;
				c.grid.setActive(!0);
				if (a.keyCode < 30)
					return c.grid.doKey({
...
eXcell_co.prototype.setValue = function (a) {
	if (typeof a == "object") {
		...
		for (var c = 0; c < b.length; c++)
			this.cell._combo.put(b[c].getAttribute("value"), b[c].firstChild ? b[c].firstChild.data : "");
		a = a.firstChild.data
	}
	if ((a || "").toString()._dhx_trim() == "")
		a = null;
		...
		for (var e = 256 / a, f = 0; f <= 256 / e; f++)
			for (var l = g.insertRow(f), i = 0; i <= 256 / e; i++)
				for (var j = 0; j <= 256 / e; j++) {
					...
					h._bg = "#" + k;
					if (this.val != null && "#" + k.toUpperCase() == this.val.toUpperCase())
...
eXcell_img.prototype.setValue = function (a) {
	...
	if (a.indexOf("^") != -1) {
		...
		if (c.length > 2 && (this.cell.lnk = c[2], c[3]))
			this.cell.trg = c[3];
		this.cell.titFl = "1"
	}
...
eXcell_price.prototype.setValue = function (a) {
	...
	var b = "green";
	a < 0 && (b = "red");
	this.setCValue("<span>&yen</span><span style='padding-right:2px;color:" + b + ";'>" + a + "</span>", a)
};
...
eXcell_dyn.prototype.setValue = function (a) {
	if (!a || isNaN(Number(a)))
		a !== "" && (a = 0);
	if (a > 0)
		var b = "green", c = "dyn_up.gif";
	else
		a == 0 ? (b = "black", c = "dyn_.gif") : (b = "red", c = "dyn_down.gif");
...
function dhtmlXGridComboObject() {
	...
	this.remove = function (a) {
		for (var b = 0; b < this.keys.length; b++)
			if (this.keys[b] ==
				a)
				return this.keys._dhx_removeAt(b), this.values._dhx_removeAt(b), !0
	};
	this.size = function () {
		for (var a = 0, b = 0; b < this.keys.length; b++)
			this.keys[b] != null && a++;
		return a
	};
	this.getKeys = function () {
		for (var a = [], b = 0; b < this.keys.length; b++)
			this.keys[b] != null && (a[a.length] = this.keys[b]);
		return a
	};
	this.save = function () {
		this._save = [];
